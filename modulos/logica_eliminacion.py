def procesar_eliminacion_eventos(
    eventos_a_eliminar_indices, eventos_planificados, recursos
):
    """
                ...
                for recurso in recursos:
                    if recurso["nombre_mostrar"] == recurso_detalle["nombre_mostrar"]:
                        ...
                        if es_combustible and cantidad > 0:
                            ...
                            # EQUIPOS: registrar liberación con cantidad
                            cantidad = recurso_detalle.get("cantidad", 1)
                            encontrado = False
                            for r in recursos_devueltos:
                                if r["nombre"] == recurso["nombre_mostrar"]:
                                    r["cantidad"] = r.get("cantidad", 0) + cantidad
                                    encontrado = True
                                    break
...
def generar_mensaje_resumen(
    eventos_eliminados, recursos_devueltos, combustible_devuelto
):
    """
    ...
    if recursos_devueltos:
        total_unidades = sum(item.get("cantidad", 1) for item in recursos_devueltos)
        equipos_unicos = len(set(item["nombre"] for item in recursos_devueltos))
        mensaje += f"\n🔄 {total_unidades} unidad(es) de equipo liberada(s) ({equipos_unicos} tipo(s))."
